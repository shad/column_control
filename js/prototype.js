...
(function(browser) {
  ...
  var timer, styleSheets = document.styleSheets || [];
  ...
  if (document.addEventListener) {     
 	  document.addEventListener("DOMContentLoaded", function() { 
 	    // ensure all stylesheets are loaded, solve Opera issue 
 	    var i = styleSheets.length; 
 	    while (i--) 
 	      if (styleSheets[i].disabled) return setTimeout(arguments.callee, 0); 
  ...
  if (browser.WebKit) { 
    timer = setInterval(function() { 
      if (/loaded|complete/.test(document.readyState) && 
          styleSheets.length == $$('style, link[rel="stylesheet"]').length)
        fireContentLoadedEvent();
    }, 0);
  } else if (browser.IE) { 
   	timer = setInterval(function() { 
